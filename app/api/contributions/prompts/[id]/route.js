...
export async function POST(request, { params }) {
  ...
  const { data: prompt } = await supabase.from('prompt_submissions').select('id').eq('id', id).eq('status', 'approved').maybeSingle();
  ...
  if (action === 'copy' || action === 'view' || action === 'save') {
    const { error } = await supabase.from('analytics_prompt_events').insert({ prompt_id: id, event_type: action, user_id: user?.id || null });
