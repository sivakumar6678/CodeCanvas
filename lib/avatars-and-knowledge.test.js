...
test('Profile username regex and avatar validation rules', () => {
  const USERNAME_REGEX = /^[a-zA-Z0-9_.\- ]{2,50}$/;

  // Valid usernames (including dots, dashes, underscores, spaces)
  assert.equal(USERNAME_REGEX.test('siva.kumar'), true);
  assert.equal(USERNAME_REGEX.test('alex_dev-123'), true);
  assert.equal(USERNAME_REGEX.test('Taylor Swift'), true);
  assert.equal(USERNAME_REGEX.test('coder.ai_pro-99'), true);

  // Invalid usernames
  assert.equal(USERNAME_REGEX.test('a'), false); // Too short (< 2)
  assert.equal(USERNAME_REGEX.test('a'.repeat(51)), false); // Too long (> 50)
  assert.equal(USERNAME_REGEX.test('user@email.com'), false); // Illegal @
  assert.equal(USERNAME_REGEX.test('bad$user!'), false); // Illegal punctuation

  // Avatar presets validation
  assert.equal(isValidAvatarId('avatar-coder'), true);
  assert.equal(isValidAvatarId('avatar-shield'), true);
  assert.equal(isValidAvatarId('avatar-cat'), true);
