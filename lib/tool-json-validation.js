...
export const REQUIRED_TOOL_FIELDS = ['id', 'name', 'slug', 'description', 'website', 'category', 'pricing'];
...
export function validateToolRecords(records, { categories = [], categoryFiles = [], existingTools = [] } = {}) {
  ...
  if (!Array.isArray(records) && (!records || typeof records !== 'object')) {
    return { valid: false, records: [], errors: ['The JSON root must be an object or array of tool objects.'], duplicates: [] };
  }
  ...
  list.forEach((tool, index) => {
    ...
    if (tool.pricing && !ALLOWED_PRICING.includes(tool.pricing)) errors.push(`${label}: pricing must be one of ${ALLOWED_PRICING.join(', ')}.`);
    ['tags', 'useCases', 'platform', 'features', 'pros', 'cons'].forEach((field) => {
    ...
    if (tool.id && existingIds.has(tool.id)) {
      const existing = existingTools.find((candidate) => candidate.id === tool.id);
    ...
    if (tool.slug && existingSlugs.has(tool.slug)) {
      const existing = existingTools.find((candidate) => candidate.slug === tool.slug);
